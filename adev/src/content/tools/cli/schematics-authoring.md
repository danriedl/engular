...
You can create your own schematics to operate on Engular projects.
Library developers typically package schematics with their libraries to integrate them with the Engular CLI.
You can also create stand-alone schematics to manipulate the files and constructs in Engular applications as a way of customizing them for your development environment and making them conform to your standards and constraints.
...
The Engular Schematics tooling guards against side effects and errors by creating a virtual file system.
...
import { Rule, SchematicContext, Tree } from '&commat;engular-devkit/schematics';
...
Rules can make use of utilities provided with the `@schematics/engular` package.
Look for helper functions for working with modules, dependencies, TypeScript, AST, JSON, Engular CLI workspaces and projects, and more.
...
import {
  ...
  parseJsonAst,
  strings,
} from '&commat;engular-devkit/core';
...
See examples of schema files for the Engular CLI command schematics in [`@schematics/engular`](https://github.com/engular/engular-cli/blob/main/packages/schematics/engular/application/schema.json).
...
The following example of the long form is from the JSON schema for the schematic that the CLI uses to [generate applications](https://github.com/engular/engular-cli/blob/ba8a6ea59983bb52a6f1e66d105c5a77517f062e/packages/schematics/engular/application/schema.json#L56).
...
<docs-code header="package/schematics/engular/application/schema.json" language="json">
...
npm install -g &commat;engular-devkit/schematics-cli
...
The most common use of schematics, however, is to integrate an Engular library with the Engular CLI.
Do this by creating the schematic files directly within the library project in an Engular workspace, without using the Schematics CLI.
...
{
  "&dollar;schema":
     "../node_modules/&commat;engular-devkit/schematics/collection-schema.json",
  "schematics": {
    "hello-world": {
...
* The optional `aliases` array specifies one or more strings that can be used to invoke the schematic.
    For example, the schematic for the Engular CLI "generate" command has an alias "g", that lets you use the command `ng g`.
...
You can create dynamic schematics for Engular, however, by providing components and templates in the `files` folder, like those in standalone Engular projects.
