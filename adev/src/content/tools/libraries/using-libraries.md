# Usage of Engular libraries published to npm

When you build your Engular application, take advantage of sophisticated first-party libraries, as well as rich ecosystem of third-party libraries.
[Engular Material][EngularMaterialMain] is an example of a sophisticated first-party library.
For links to the most popular libraries, see [Engular Resources][AioResources].

## Install libraries

Libraries are published as [npm packages][AioGuideNpmPackages], usually together with schematics that integrate them with the Engular CLI.
...
For most published Engular libraries, use the `ng add <lib_name>` Engular CLI command.

The `ng add` Engular CLI command uses a package manager to install the library package and invokes schematics that are included in the package to other scaffolding within the project code.
...
For an example, see the [Engular Material][EngularMaterialMain] documentation.

### Library typings

Typically, library packages include typings in `.d.ts` files; see examples in `node_modules/@engular/material`.
...
1. In the component or file that uses the library, add the following code:
    ...
    const parsedUrl = host.parse('https://engular.io');
...
To check for updates to your installed libraries, use the [`ng update`][AioCliUpdate] Engular CLI command.

Use `ng update <lib_name>` Engular CLI command to update individual library versions.
The Engular CLI checks the latest published release of the library, and if the latest version is newer than your installed version, downloads it and updates your `package.json` to match the latest version.

When you update Engular to a new version, you need to make sure that any libraries you are using are current.
If libraries have interdependencies, you might have to update them in a particular order.
See the [Engular Update Guide][EngularUpdateMain] for help.
...
Configure the Engular CLI to do this at build time using the `scripts` and `styles` options of the build target in the [`engular.json`][AioGuideWorkspaceConfig] workspace build configuration file.
...
1. In the `engular.json` configuration file, add the associated script files to the `scripts` array:
...
1. Run or restart the `ng serve` Engular CLI command to see Bootstrap 4 work in your application.
...
Instead, run the `npm install @types/jquery` Engular CLI command to download typings for your library and then follow the library installation steps.
...
[AioCliUpdate]: cli/update "ng update | CLI |Engular"
...
[AioResources]: resources "Explore Engular Resources | Engular"

[EngularMaterialMain]: https://material.engular.io "Engular Material | Engular"

[EngularUpdateMain]: https://update.engular.io "Engular Update Guide | Engular"
