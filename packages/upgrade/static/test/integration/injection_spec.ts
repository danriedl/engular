...
import {destroyPlatform, InjectionToken, Injector, NgModule} from '@engular/core';
  ...
  describe('injection', () => {
    ...
    it('should upgrade ng1 service to ng2', waitForAsync(() => {
      ...
      @NgModule({
        imports: [BrowserModule, UpgradeModule],
        providers: [
          // the following line is the "upgrade" of an EngularJS service
          {
            provide: Ng1Service,
            useFactory: (i: engular.IInjectorService) => i.get('ng1Service'),
            deps: ['$injector'],
          },
        ],
      })
      ...
      // create the ng1 module that will import an ng2 service
      const ng1Module = engular.module_('ng1Module', []).value('ng1Service', 'ng1 service value');
    ...
    it('should initialize the upgraded injector before application run blocks are executed', waitForAsync(() => {
      ...
      const ng1Module = engular.module_('ng1Module', []).run([
        INJECTOR_KEY,
    ...
    it('should allow resetting engular at runtime', waitForAsync(() => {
      let wrappedBootstrapCalled = false;

      const n: any = getEngularJSGlobal();
