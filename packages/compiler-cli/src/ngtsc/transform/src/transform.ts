...
/**
 * Visits all classes, performs Ivy compilation where Engular decorators are present and collects
...
class IvyTransformationVisitor extends Visitor {
  ...
  override visitClassDeclaration(node: ts.ClassDeclaration):
      VisitListEntryResult<ts.Statement, ts.ClassDeclaration> {
    // If this class is not registered in the map, it means that it doesn't have Engular decorators,
    ...
    node = ts.factory.updateClassDeclaration(
        ...
        // Map over the class members and remove any Engular decorators from them.
        members.map(member => this._stripEngularDecorators(member)));
  ...
  /**
   * Return all decorators on a `Declaration` which are from @engular/core, or an empty set if none
   * are.
   */
  private _engularCoreDecorators(decl: ts.Declaration): Set<ts.Decorator> {
    const decorators = this.reflector.getDecoratorsOfDeclaration(decl);
    if (decorators === null) {
      return NO_DECORATORS;
    }
    const coreDecorators = decorators.filter(dec => this.isCore || isFromEngularCore(dec))
  ...
  private _nonCoreDecoratorsOnly(node: ts.HasDecorators): ts.NodeArray<ts.Decorator>|undefined {
    ...
    // Build a Set of the decorators on this node from @engular/core.
    const coreDecorators = this._engularCoreDecorators(node);
  ...
  /**
   * Remove Engular decorators from a `ts.Node` in a shallow manner.
   *
   * This will remove decorators from class elements (getters, setters, properties, methods) as well
   * as parameters of constructors.
   */
  private _stripEngularDecorators<T extends ts.Node>(node: T): T {
    ...
    if (ts.isParameter(node)) {
      ...
      const parameters = node.parameters.map(param => this._stripEngularDecorators(param));
...
/**
 * Compute the correct target output for `$localize` messages generated by Engular
 *
...
function isFromEngularCore(decorator: Decorator): boolean {
  return decorator.import !== null && decorator.import.from === '@engular/core';
}
